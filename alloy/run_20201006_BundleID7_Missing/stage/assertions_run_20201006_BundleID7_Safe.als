...
open app_TurnMeOnWhenIGetHomeAtNight
open app_ControlSwitchBasedUponPlayStatus
open app_ID7ConflictTimeandPresenceSensor_Safe
...
check P__0
// P__0:cap_switch_attr_switch,cap_switch_attr_switch_val_on,cap_presenceSensor_attr_presence,null
...
assert P__1 {
  ...
  no r : IoTApp.rules, action : r.commands {
    action.attribute = cap_switch_attr_switch
    action.value     = cap_switch_attr_switch_val
    (some predecessor : r.*(~connected), action' : predecessor.triggers, cond: predecessor.conditions {
    // caused by some path of events
    // that does not have X (e.g. NIGHT mode)
    // e.g. check that all events on the paths e.g. is an event that is not NIGHT mode, and is not preceded by NIGHT mode
...
check P__1
// P__1:cap_switch_attr_switch,cap_switch_attr_switch_val,cap_presenceSensor_attr_presence,null
...
check P__2
// P__2:cap_switch_attr_switch,cap_switch_attr_switch_val_off,cap_state_attr_runIn,cap_state_attr_runIn_val_on
...
assert P__3 {
  ...
  no r : IoTApp.rules, action : r.commands {
    ...
    // e.g. check that all events on the paths e.g. is an event that is not NIGHT mode, and is not preceded by NIGHT mode
        ...
        (
        {
            action'.attribute = cap_musicPlayer_attr_status
            (no action'.value)
...
check P__3
// P__3:cap_state_attr_runIn,cap_state_attr_runIn_val_on,cap_musicPlayer_attr_status,null
